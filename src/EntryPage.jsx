...
export default function EntryPage() {
  ...
  async function onCreateHeadache(ev) {
    ev.preventDefault()
    setError(null)
    try {
  ...
  async function onCreateEvent(ev) {
    ev.preventDefault()
    setError(null)
    try {
        ...
        <form onSubmit={onCreateHeadache} className="grid sm:grid-cols-3 gap-3">
          <label className="flex flex-col gap-2 sm:col-span-2">
            ...
            <input
              ...
              style={{ ['--percent']: `${(newHeadache.severity / 10) * 100}%`, ['--sev-color']: getSeverityColor(newHeadache.severity) }}
            />
          </label>
